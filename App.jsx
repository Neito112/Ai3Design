...
const createBlankCanvas = (ratioId) => {
    return new Promise((resolve) => {
        ...
        // Lấy kích thước chuẩn, nhưng nhân đôi để có độ nét cao ngay từ đầu (giống logic Max Canvas)
        let width = 2048; 
        let height = 2048;
        ...
        // Tô nền trắng (để AI có chỗ vẽ)
        ctx.fillStyle = '#FFFFFF';
        ctx.fillRect(0, 0, width, height);

        // Chuyển thành File object để tái sử dụng logic compressImage
        canvas.toBlob((blob) => {
            const file = new File([blob], "blank_canvas.png", { type: "image/png" });
            // Trả về cấu trúc giống input file bình thường
            resolve({
                file: file,
      ...
      img.onload = () => {
        ...
        if (targetRatioId && RATIO_CONFIG[targetRatioId]) {
           ...
           if (targetRatio >= 1) { 
            ...
            if (sourceWidth > sourceHeight) {
                if (sourceWidth > MAX_DIMENSION) {
                    finalWidth = MAX_DIMENSION;
                    finalHeight = Math.round(sourceHeight * (MAX_DIMENSION / sourceWidth));
                }
            } else {
                if (sourceHeight > MAX_DIMENSION) {
                    finalHeight = MAX_DIMENSION;
                    finalWidth = Math.round(sourceWidth * (MAX_DIMENSION / sourceHeight));
                }
            }
        }
        ...
        if (targetRatioId) {
            if (taskType === 'face' || taskType === 'sketch' || taskType === 'creation') {
                // --- LOGIC OUTPAINT/CREATION TRÊN CANVAS LỚN ---
                ...
                // 4. Vẽ ảnh gốc vào giữa (Với task 'creation', ảnh gốc là trang trắng, vẽ vào cũng ko sao)
                ctx.drawImage(img, x, y, drawW, drawH);

            } else {
                // Logic cũ cho Edit/Batch
...
const analyzeImageDelta = async (orgFile, resBase64) => {
    ...
    const prompt = `
        ...
        GOAL: Describe the "Added Elements" or "Style Changes" in specific detail.
        REQUIREMENTS: Identify Exact Object added, Exact Style. DO NOT describe original content.
        OUTPUT: A concise instruction string.
    `;
...
// --- API 2: UNIVERSAL GENERATION (Tab 1, 2, 3, 4, 5) ---
const generateMultimodalImage = async (prompt, files, taskType, ratioId = null, extraContext = null) => {
  ...
  if (taskType === 'batch_execute' && extraContext) {
      ...
      imageParts = [
          { inlineData: { mimeType: targetCompressed.mimeType, data: targetCompressed.data } }, 
          { inlineData: { mimeType: 'image/png', data: res1Base64 } }                           
      ];

      const instr = extraContext.instructions;
      systemContext = `
        ROLE: Strict Visual Transfer Engine.
        INPUTS: [IMAGE 1 - CANVAS], [IMAGE 2 - ASSET SOURCE].
        INSTRUCTION: "${instr}"
        RULES:
        1. STRUCTURE LOCK: Preserve IMAGE 1 layout/pose 100%.
        2. ASSET CLONING: Copy style/objects from IMAGE 2 to IMAGE 1.
        3. FAIL STATE: If output background matches IMAGE 2, you fail.
      `;

  } else {
      // --- STANDARD MODES (Edit, Sketch, Face, CREATION) ---
      ...
      const commonInstructions = `
        GENERAL RULES: 
        1. **HIGH RESOLUTION**: The output must be crisp, sharp, and high-fidelity. 
        2. **NO BLUR**: Ensure all details are in focus.
        3. **PHOTOREALISTIC**: Unless specified otherwise, aim for DSLR quality.
        ${ratioInstruction}
      `;

      if (taskType === 'creation') {
        // --- LOGIC TẠO ẢNH TRÊN NỀN GIẤY (TAB 1) ---
        // Input là 1 tờ giấy trắng (blank canvas) đúng tỉ lệ.
        systemContext = `
          ${commonInstructions}
          ROLE: Master Painter & Digital Artist.
          
          **INPUT**: A blank canvas.
          
          **TASK**: 
          - IGNORE the white pixels. Treat the canvas as a viewport.
          - GENERATE a completely new image inside this viewport based on the prompt: "${prompt}".
          - **FILL THE ENTIRE CANVAS**: Do not leave any white borders.
          - **COMPOSITION**: Ensure the subject fits perfectly within this aspect ratio.
        `;
        ...
        systemContext = `
          ...
          TASK 1: OUTPAINTING. Fill empty canvas space naturally. NO white borders.
          TASK 2: RENDERING. Convert sketch to Hyper-Realistic Photo.
        `;
        ...
        systemContext = `
          ...
          TASK:
          1. OUTPAINTING: Fill surrounding white space.
          2. FACE ID: Transform subject based on: "${prompt}". Keep identity consistent.
        `;
      }
  }
  ...
  if (!imagePart) {
      ...
      throw new Error(textPart?.text || "AI không trả về ảnh.");
  }
...
const Lightbox = ({ images, initialIndex, onClose, onDownload }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-xl animate-in fade-in duration-200">
      ...
      {images.length > 1 && (
        <>
          <button onClick={handlePrev} disabled={currentIndex === 0} className={`absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full transition-all z-50 ${currentIndex === 0 ? 'bg-white/5 text-white/20 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-white/20'}`}><ChevronLeft size={32} /></button>
          <button onClick={handleNext} disabled={currentIndex === images.length - 1} className={`absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full transition-all z-50 ${currentIndex === images.length - 1 ? 'bg-white/5 text-white/20 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-white/20'}`}><ChevronRight size={32} /></button>
        </>
      )}
      ...
      <div className={`relative max-w-[90vw] max-h-[85vh] transition-opacity duration-200 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
        {isLoading ? (
          ...
          <img src={currentUrl} alt={`Result ${currentIndex}`} className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl" />
        )}
      </div>
                      ...
                      <div key={idx} onClick={() => onViewFull(displayList, idx)} className={`aspect-square rounded-xl border relative overflow-hidden group cursor-pointer transition-all ${item.status === 'done' ? (idx === 0 ? 'border-yellow-400' : 'border-white/20 hover:border-blue-400') : 'border-white/5 bg-white/5'}`}>
                         {item.status === 'done' ? (
                            <>
                              ...
                              <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"><Maximize2 size={16} className="text-white drop-shadow-md" /></div>
                              <button onClick={(e) => { e.stopPropagation(); onRegenerateSingle(idx); }} className="absolute bottom-2 left-2 p-1.5 bg-white/10 hover:bg-blue-500 text-white rounded-full backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 z-10" title="Tạo lại ảnh này"><RotateCw size={14} /></button>
                              {idx === 0 && <div className="absolute top-2 right-2 bg-yellow-500 text-black text-[9px] px-1.5 py-0.5 rounded font-bold shadow-sm flex items-center gap-1"><Sparkles size={10} fill="black" /> REFERENCE</div>}
                            </>
                         ) : item.status === 'pending' ? (
                            <div className="w-full h-full flex flex-col items-center justify-center"><div className="w-6 h-6 border-2 border-blue-500/30 border-t-blue-500 rounded-full animate-spin mb-2" /><span className="text-[10px] text-white/40">{idx === 0 ? 'Đang tạo mẫu...' : 'Đang đồng bộ...'}</span></div>
                         ) : (
                            <div className="w-full h-full flex flex-col items-center justify-center text-red-400/50 relative group"><AlertCircle size={24} /><button onClick={(e) => { e.stopPropagation(); onRegenerateSingle(idx); }} className="absolute bottom-2 p-1.5 bg-white/10 hover:bg-blue-500 text-white rounded-full backdrop-blur-md transition-all z-10" title="Thử lại"><RotateCw size={14} /></button></div>
                         )}
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className={`flex-1 bg-black/20 rounded-2xl border border-white/10 relative overflow-hidden flex items-center justify-center min-h-[300px] transition-all duration-500 ease-out ${resultImage && !isGenerating ? 'cursor-zoom-in hover:scale-[1.02] hover:-translate-y-2 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.6)] hover:border-white/30' : ''}`} onClick={() => resultImage && onViewFull([resultImage], 0)}>
        {isGenerating ? (
          <div className="flex flex-col items-center justify-center text-blue-400 animate-pulse"><Sparkles size={48} className="mb-4 animate-spin-slow" /><span className="text-lg font-medium tracking-wider">AI đang xử lý...</span></div>
        ) : error ? (
           <div className="flex flex-col items-center justify-center text-red-400 text-center p-4"><AlertCircle size={48} className="mb-2" /><p className="font-bold">Đã xảy ra lỗi</p><p className="text-sm opacity-80 mt-1 max-w-md">{error}</p></div>
        ) : resultImage ? (
          <img src={resultImage} alt="AI Result" className="w-full h-full object-contain animate-in fade-in zoom-in duration-500 drop-shadow-md" />
        ) : (
          <div className="text-white/30 flex flex-col items-center"><ImageIcon size={64} className="mb-2 opacity-50" /><p>Kết quả hiển thị tại đây</p></div>
        )}
      </div>
        ...
        {history.length === 0 ? (
          ...
          <div className="flex gap-2 flex-1 min-h-0 overflow-x-auto custom-scrollbar">
            {history.map((item, idx) => {
               ...
               return (
                  <div key={idx} className="relative w-24 shrink-0 rounded-lg overflow-hidden border border-white/10 cursor-pointer group hover:border-white/40 transition-all h-full bg-black/30" onClick={() => isFolder ? onViewBatch(item) : onViewFull(item)}>
                    ...
                    {isFolder && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none"><FolderOpen size={24} className="text-white drop-shadow-lg" /><div className="absolute bottom-1 right-1 text-[9px] font-bold bg-black/60 px-1 rounded text-white border border-white/10">{item.items?.length || 0}</div></div>)}
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors" />
                    <button onClick={(e) => { e.stopPropagation(); onRemoveHistory(idx); }} className="absolute top-1 right-1 p-1 bg-red-500/90 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 shadow-md backdrop-blur-sm z-10"><X size={10} strokeWidth={3} /></button>
                  </div>
               );
            })}
          </div>
...
export default function AIArtApp() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Escape') setLightboxData({ ...lightboxData, isOpen: false });
    };
  ...
  // --- CẬP NHẬT HÀM DOWNLOAD IMAGE (MAX UPSCALE) ---
  const downloadImage = (url) => {
    ...
    img.onload = () => {
        ...
        // LUÔN LUÔN UPSCALE LÊN MAX (4K) CHO MỌI TAB
        const MAX_UPSCALE_SIZE = 4096; // Kích thước trần
        
        let targetW = img.width;
        let targetH = img.height;
        let shouldUpscale = false;

        // Nếu ảnh nhỏ hơn max size, phóng to lên cho đến khi chạm trần 4K
        if (targetW < MAX_UPSCALE_SIZE && targetH < MAX_UPSCALE_SIZE) {
             ...
             const scale = Math.min(scaleW, scaleH); // Giữ tỉ lệ
             
             // Chỉ upscale nếu scale > 1 (ảnh nhỏ hơn 4K)
             if (scale > 1) {
      ...
      <div className="flex-1 flex overflow-hidden z-10">
        <div className="w-full md:w-[360px] border-r border-white/5 bg-black/10 backdrop-blur-sm flex flex-col h-full">
          <div className="flex-1 flex flex-col p-5 gap-5 min-h-0">
            <div className="shrink-0"><h2 className="text-xl font-semibold text-white/90">{TABS.find(t => t.id === activeTab)?.label}</h2><p className="text-xs text-white/40 mt-1">{activeTab === 5 ? 'Sửa 1 ảnh, áp dụng cho tất cả.' : 'AI Creative Suite.'}</p></div>
            <div className="shrink-0 space-y-2"><label className="text-[10px] font-bold text-white/40 uppercase flex justify-between">Prompt {activeTab === 5 && "(Áp dụng cho ảnh đầu tiên)"}<span className="text-white/20">{prompt.length}/500</span></label><div className="relative group"><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={activeTab === 2 || activeTab === 5 ? "VD: Thêm hiệu ứng màu film, làm nét ảnh..." : "VD: Một chú mèo máy futuristic..."} className="w-full h-32 bg-black/20 border border-white/10 rounded-xl p-3 text-white text-sm focus:border-blue-500/50 focus:bg-black/30 outline-none resize-none transition-all placeholder:text-white/20" /><div className="absolute bottom-2 right-2"><Wand2 size={14} className="text-white/20" /></div></div></div>
